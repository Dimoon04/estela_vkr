  ...
  <div id="body">
      <div class="wrapper">
        <form action="">
          <h1>Регистрация</h1>
          <div class="input-box">
            <input v-model="authRegister.txtEmail" type="text" placeholder="Имя пользователя" required>
            <i class='bx bx-user'></i>
          </div>
          <div class="input-box">
            <input v-model="authRegister.txtPassword" type="password" placeholder="Введите пароль" required>
            <i class='bx bxs-lock-alt'></i>
          </div>
            <button @click="login()" class="btn">Зарегистрироваться</button>
          <div class="register-link">
            <p>У вас есть аккаунт? <router-link to="/login">Войти</router-link></p><!--вставить ссылку-->
          </div>
        </form>
      </div>
  </div>
